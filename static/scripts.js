...
// Загружает список персонажей текущего пользователя
async function loadCharacters() {
  const res = await fetchJSON('/api/chars');
  const panel = document.getElementById('panel');

  // Удаляем старые карточки (кроме блока создания)
  [...panel.querySelectorAll('.char-block')].forEach(el => el.remove());

  if (res.ok && res.chars.length) {
  res.chars.forEach(char => {
    const displayName = char.name || char.sheet;
    const div = document.createElement('div');
    div.classList.add('block', 'char-block');
    div.innerHTML = `
      <a href="/chars/${encodeURIComponent(char.sheet)}" style="text-decoration:none;color:inherit;">
        <div><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPtCBdlq4hR6jNFFtkxEYrgJoOgnsMgKp3PQ&s"></div>
        <div><b>${char.sheet}</b></div>
      </a>
    ...
    div.querySelector('.rename-btn').addEventListener('click', e => {
      e.preventDefault();
      renameCharacter(char.sheet);
    });
    div.querySelector('.delete-btn').addEventListener('click', e => {
      e.preventDefault();
      deleteCharacter(char.sheet);
    });
    panel.insertBefore(div, panel.lastElementChild);
  });
} 
else {
...
async function deleteCharacter(name) {
  ...
  const res = await fetchJSON(`/api/chars/${name}`, { method: 'DELETE' });
  if (res.ok) {
    await loadCharacters();
  } else {
...
async function renameCharacter(oldName) {
  const newName = prompt(`Rename "${oldName}" to:`);
  if (!newName) return;

  const res = await fetchJSON(`/api/chars/${oldName}`, {
    method: 'PUT',
  ...
  if (res.ok) {
    ...
    alert(res.error || 'Error renaming');
  }
}
